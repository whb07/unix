...
void readi(struct inode *aip) {
  ...
  if ((ip->i_mode & IFMT) == IFCHR) {
    DeviceCode *dev;
    dev = (DeviceCode *) ip->i_addr[0];
    (*cdevsw[dev->d_major].d_read)(ip->i_addr[0]);
    return;
  }

  do {
    lbn = bn = lshift(u.u_offset, -9);
    on = (int) u.u_offset[1] & 0777;
...
void writei(struct inode *aip) {
  ...
  if ((ip->i_mode & IFMT) == IFCHR) {
    DeviceCode *dev;
    dev = (DeviceCode *) ip->i_addr[0];
    (*cdevsw[dev->d_major].d_write)(ip->i_addr[0]);
    return;
  }
  if (u.u_count == 0)
    return;
  ...
  do {
    ...
    if (u.u_error != 0)
      brelse(bp);
    else if (((int) (u.u_offset[1]) & 0777) == 0)
      bawrite(bp);
    else
      bdwrite(bp);
