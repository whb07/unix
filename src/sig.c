...
int core(void)
{
  ...
  u.u_uid == u.u_ruid)
  {
    ...
    u.u_segflg = 1;
    writei(ip);
    s = ((struct proc *) u.u_procp)->p_size - USIZE;
    estabur(0, s, 0, 0);
    u.u_base = 0;
    u.u_count = s * 64;
    u.u_segflg = 0;
...
int grow( char *sp){
  ...
  if (sp >= -u.u_ssize * 64)
    return (0);
  si = ldiv(-(int)sp, 64) - u.u_ssize + SINCR;
  if (si <= 0)
    return (0);
...
void ptrace(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
    if (p->p_stat == SSTOP && p->p_pid == u.u_arg[0] && p->p_ppid == u.u_procp->p_pid)
      goto found;
    u.u_error = ESRCH;
    return;
